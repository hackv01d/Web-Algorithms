...
let minWayPoints = [];
let minWayNumber = Infinity;
let distancesGens = [];
let generation = [];
let arrayGens = [];
let chromosome;
// перестановка родителей в случайном порядке
function randomChangePlace(oldPlacement) {
...
function rebirth() {
    ...
    for (let i = 0; i < generation.length; i += 2) {
        ...
        firstParent.forEach((gene) => {
            if (!secondChild.includes(gene))
                secondChild.push(gene);
        });
        secondParent.forEach((gene) => {
            if (!firstChild.includes(gene))
                firstChild.push(gene);
        });
